...
export class GiftVoucher extends Component {
    ...
    initializeEventListeners() {
        ...
        if (giftForHimBtn) {
            ...
            giftForHimBtn.addEventListener('click', this.handleGiftForHimClick);
        }

        // Gift For Her button click event
        const giftForHerBtn = document.getElementById('giftForHerBtn');
        if (giftForHerBtn) {
            giftForHerBtn.addEventListener('click', this.handleGiftForHerClick);
        }
    ...
    handleGiftForHimClick = (event) => {
        event.preventDefault();
         // redirection code for Him
        // window.location.href = '/tenStore';
    }

    handleGiftForHerClick = (event) => {
        event.preventDefault();
        // redirection code for Her
        // window.location.href = '/tenStore';
    }

    handleGiftNowClick = (event) => {
        event.preventDefault();
        // Show first time purchase question popup
        this.showFirstTimeQuestionPopup("Gift Voucher");

    }

    showFirstTimeQuestionPopup(title) {
        // Get popup from XML
        ...
        if (popup) {
            ...
            if (paragraph) {
                paragraph.textContent = "Is this your first time purchasing a gift from Ten Spa?";
